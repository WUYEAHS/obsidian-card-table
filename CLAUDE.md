...
3. **卡片的身分是「主題 + 第一行內容」**(`k.基鍵`,1.6.1 起,見「筆記格式」)。
...
**設定頁分大標**(1.6.1):更新內容(最上面)→ 一般 → 輔助使用(1.7.6,只有「看板大小」)→ 時間篩選 → 卡片外觀 → 新增與編輯 → 動作後跳轉 → 筆記格式 → 支持。
...
- 內容行的符號照使用者打的寫(見「筆記格式」);讀的時候 `去符()` / `內文之()` 有沒有符號都吃得下。
...
## 筆記格式(1.6.1 定案,1.6.3 起主題 / 指派人換寫法,ADR 1.6.3-01)

**格式的正本是 README 的「所有寫法」表**,寫任何筆記範例之前先看那張表(2026-09-25:照這裡過時的範例寫出舊格式的示範筆記,被使用者抓到;1.7.6-D2 改成現在的寫法)。
主題是 `#主題`(最多 3 個,空格換 `-`)、指派人是 `@名字`;`[主題]`、`#指派人`(在名單裡的)是舊寫法,照讀、改到才轉。
...
- 第一行順序:`[pin:: on]` → `#主題`(沒有主題時是第一行內容)→ 日期 → `[repeat::]` → `@指派人` → 其他標籤 → **不認得的欄位原樣留著**(`拆首行().欄`)。
...
- **看板大小 = CSS `zoom`**(`.tk-board:not(.tk-嵌卡)`,`--tk-板倍`)。設定 → 輔助使用 → 看板大小 `設定.看板大小.桌/手`(80–120%,每 1%;**80% = 原本 = zoom 1**,zoom = 值 / 80;預設桌 100 = 1.25、手 80)。使用者**只能調這一個**(CR-1.7.6-03)。⚠⚠ zoom 底下 **`getBoundingClientRect` 是螢幕上的(放大後),`offsetWidth/Height`、`scrollTop`、style 的 px 是放大前的**。
  拿 rect 的數字去寫 style / scrollTop 一律先 `/ this.屏倍()`,或直接改量 `offsetHeight`。浮在 `document.body` 的面板(選單、日期、主題建議)不在放大裡,用 rect 定位是對的。
  定寬的 `max-width` px 要除倍數(表格寬度才不變);% 不用除。
- **圖示大小 = `transform: scale(var(--tk-圖倍))`**(不佔版面)。行內寫了 transform 的 svg 不受影響。✎ ⋯、已逾期 / 週期少 0.1(使用者要的)。
- **點擊範圍 = `::after`**(`--tk-點-*` px),畫面不變。被父層 `overflow:hidden` 切的撐不出去(主題膠囊、常用主題箭頭、日期)—— 要動就是改版面。
- 三組變數都由 `插件.套點擊()` 寫到 body:圖示大小、點擊範圍是固定值(`點擊值` 常數,桌 / 手各一組,**不是設定**,1.7.6 開發中存進 data.json 的 `點擊` 在 onload 刪掉);看板大小讀設定。測試工具:measure 開頭把倍數歸 1(數字都是 100% 的)、check.js 逐元素掃時點擊範圍和圖示倍數歸回、放大時容許 2px、check4 的 390 / 360 用手機那組。
- 📌 已釘不畫底色,只靠 accent 色。
